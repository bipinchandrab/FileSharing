        ...
        while (true) {
            ...
            try {
                ...
                if (sArray[0].equals("PPP2")) { //check first split is equals to currrent peerID
                    ///file transfer
                    //6789 = PP21 reception port
                    // sArray[1] = user request
                    if (!sArray[1].equals("cannot be found")) {
                        System.out.println("Receied msg from PPP2");
                        new SendFiles(ms, group, port, myid, sArray[0], 6789, sArray[1], txtAreaAnswer);
                    }
                }

                if (sArray[0].equals("PPP1")) { //check first split is equals to currrent peerID
                    ///file transfer
                    //6779 = PPP1 reception port
                    // sArray[1] = user request
                    if (sArray[1].equals("cannot be found")) {
                        System.out.println("Error msg from PPP1");
